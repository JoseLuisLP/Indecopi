...
setwd("D:/CURSOS_VIRTUALES/BEST/Sesion3/MDv01sesion3_parte-practica-master/1 Indecopi")
...
parcats(p, marginal_histograms = TRUE, data_input = barreras3, labelfont = list(size = 10, color = "orange"))

plot_condensation(barreras3, first =TIPO.DE.ACTUACIÓN)

Tabla1<-barreras3 %>%
  group_by(TIPO_ENTIDAD,TIPO_BARRERA)%>%
  summarise(Total=n())%>%
  arrange(desc(Total))
  
# Tarea final: un proyecto